const OVERLAY_NAMES = ['scc', 'cycles', 'shortest'];
...
export const DEFAULT_VIEWER_URL_STATE = {
  expression: '',
...
function createOverlaySelection(partial = {}) {
  return {
...
function parseOverlays(value) {
  ...
  value
    ...
    .filter((token) => OVERLAY_NAMES.includes(token))
    .forEach((token) => {
      result[token] = true;
    });
  return result;
...
export function decodeViewerStateFromSearch(source) {
...
export function encodeViewerStateToSearch(state) {
...
export function encodeViewerStateToUrl(state, base) {
