...
const flatten = (e: Expr): Expr => {
  if (e.type === 'add') {
    ...
    for (const a of e.args) {
      const s = flatten(a);
      if (s.type === 'add') args.push(...s.args); else args.push(s);
    }
  ...
  if (e.type === 'mul') {
    ...
    for (const a of e.args) {
      const s = flatten(a);
      if (s.type === 'mul') args.push(...s.args); else args.push(s);
    }
...
function simplifyOnce(e: Expr): Expr {
  e = flatten(e);
  switch (e.type) {
    case 'rat': return { ...e, value: R.make(e.value.n, e.value.d) };
    case 'add': {
      ...
      for (const t of terms) {
        if (isRat(t)) {
          acc = acc ? R.add(acc, t.value) : t.value;
        } else {
          rest.push(t);
        }
      }
    ...
    case 'pow': {
      ...
      if (isRat(Ls) && isRat(Rs) && Rs.value.d === 1n) {
        return mkRat(R.powInt(Ls.value, Rs.value.n).n, R.powInt(Ls.value, Rs.value.n).d)  // placeholder will be replaced below
      }
    ...
    case 'cbrt': {
      ...
      if (isRat(A)) {
        const num = A.value.n; const den = A.value.d;
        // Negative under sqrt stays symbolic
        if (e.type === 'sqrt' && num < 0n) return { ...e, arg: A };
        const root = e.type === 'sqrt' ? 2n : 3n;
        const perfect = (root === 2n
          ? (x: bigint) => { if (x < 0n) return null; const r = R.bigintSqrt(x); return r*r===x ? r : null; }
          : (x: bigint) => { const r = R.bigintCbrt(x); return r*r*r===x ? r : null; });
...
export const print = (e: Expr): string => {
  ...
  const p = (x: Expr, parent = 0): string => {
    switch (x.type) {
      case 'rat': return R.toString(R.make(x.value.n, x.value.d));
      case 'add': return x.args.map(a => p(a,1)).join(' + ');
      case 'mul': return x.args.map(a => p(a,2)).join(' * ');
      case 'sub': {
        const l = p(x.left,1), r = p(x.right,1);
        return `${l} - ${r}`;
      }
      case 'div': {
        const l = p(x.left,2), r = p(x.right,2);
        return `${need(2,x.left)?'('+l+')':l} / ${need(2,x.right)?'('+r+')':r}`;
      }
      case 'pow': {
        const l = p(x.left,3), r = p(x.right,3);
        return `${need(3,x.left)?'('+l+')':l} ^ ${need(3,x.right)?'('+r+')':r}`;
      }
      case 'sqrt': return `sqrt(${p(x.arg,4)})`;
      case 'cbrt': return `cbrt(${p(x.arg,4)})`;
    }
  };
  return p(e, 0);
};
